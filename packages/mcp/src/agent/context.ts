...
export function buildAgentPreamble(opts: ContextOptions = {}): string {
  ...
  if (opts.includePipelineHint !== false) {
    ...
    lines.push('  • Review / critique / "how does this look" → reframe-critic skill.');
    lines.push('  • Vague ask / ≤10 words / brand named but scope vague → reframe-enhance skill (runs the interview protocol — 2-3 priority questions max, never an interrogation).');
    lines.push('  • Brand named or "rebrand" intent → reframe-brand skill (load DESIGN.md before any HTML work).');
    lines.push('  • Multi-page / site / "home + pricing + about" → reframe-site-loop skill (baton pattern, one page per turn).');
    lines.push('  • Export to React / TSX / component library → reframe-to-react skill.');
    lines.push('  • Test the Platform UI itself (dev-side QA, not design work) → designer-qa skill + mcp__reframe__reframe_ui.');
    lines.push('');
    ...
    lines.push('');
    lines.push('The 7 reframe MCP tools (callable DIRECTLY by their full prefixed name — do NOT wrap in ToolSearch, they are first-class tools):');
    ...
    lines.push('  mcp__reframe__reframe_ui       — Playwright-backed Platform UI automation (open / act / probe / screenshot / scene / etc.) — used by designer-qa skill and any "eyeball the rendered scene" flow from reframe-critic');
    lines.push('');
