        ...
        .then(function(data) {
          ...
          items.innerHTML = data.scenes.map(function(scene) {
            ...
            return '<div class="variant-card' + (isActive ? ' variant-active' : '') + '" data-variant-scene="' + scene.id + '" style="'
              ...
              + '<div style="width:136px;height:80px;border-radius:4px;overflow:hidden;background:var(--surface-sunken) url(/cover/' + scene.id + '.svg) center/cover no-repeat;position:relative">'
              + '<img src="/thumbnail/' + scene.id + '.png?scale=1" loading="lazy" style="width:100%;height:100%;object-fit:cover;opacity:0;transition:opacity 240ms ease" onload="this.style.opacity=1" onerror="this.remove()">'
